...
#include "compute_aveorientorder_atom.h"
...
ComputeOrientAveOrderAtom::ComputeOrientAveOrderAtom(LAMMPS *lmp, int narg, char **arg) :
...
ComputeOrientAveOrderAtom::~ComputeOrientAveOrderAtom()
{
  if (copymode) return;

  memory->destroy(qnarray);
  //memory->destroy(qnm_all);
...
void ComputeOrientAveOrderAtom::init()
{
...
void ComputeOrientAveOrderAtom::init_list(int /*id*/, NeighList *ptr)
{
  list = ptr;
}
...
void ComputeOrientAveOrderAtom::compute_peratom()
{
  ...
  if (atom->nmax > nmax) {
    ...
    //memory->create(qnm_all, nmax, nncol, "orientorder/atom:qnm_all");
    array_atom = qnarray;
  }
  ...
  if (aveflag) {
    ...
    for (ii = 0; ii < inum; ii++) {
      ...
      if (mask[i] & groupbit) {
        ...
        if (nnn > 0) {
          select3(nnn, ncount, distsq, nearest, rlist); // BXU: not sure about if nearest has changed the order
          ncount = nnn;
        }
...
double ComputeOrientAveOrderAtom::memory_usage()
{
...
void ComputeOrientAveOrderAtom::select3(int k, int n, double *arr, int *iarr, double **arr3)
{
...
void ComputeOrientAveOrderAtom::calc_boop(double **rlist, int ncount, double qn[], int qlist[],
                                       int nqlist)
{
...
double ComputeOrientAveOrderAtom::polar_prefactor(int l, int m, double costheta)
{
...
double ComputeOrientAveOrderAtom::associated_legendre(int l, int m, double x)
{
  if (l < m) return 0.0;
...
void ComputeOrientAveOrderAtom::init_clebsch_gordan()
{
